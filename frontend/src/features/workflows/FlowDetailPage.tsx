...
import { CatalogSelect, ErrorState, LoadingState, StatusBadge } from '../../shared/ui'
...
interface InheritedTaskDefault {
  type: string
  effective: unknown
  origins: unknown
}
...
function inputProperties(inputSchema: Record<string, unknown>): Record<string, FlowInputSchemaProperty> {
  const result: Record<string, FlowInputSchemaProperty> = {}
  const properties = inputSchema.properties
  if (!isRecord(properties)) return result
  for (const [id, value] of Object.entries(properties)) {
    if (!isRecord(value)) continue
    const property: FlowInputSchemaProperty = {}
    if (typeof value.type === 'string' || (Array.isArray(value.type) && value.type.every((item) => typeof item === 'string'))) property.type = value.type
    if (Array.isArray(value.enum)) property.enum = value.enum
    if (typeof value.title === 'string') property.title = value.title
    ...
    if (Array.isArray(value.examples)) property.examples = value.examples
    if (typeof value.writeOnly === 'boolean') property.writeOnly = value.writeOnly
    const input = value['x-amesh-input']
    if (isRecord(input) && typeof input.type === 'string' && typeof input.sensitive === 'boolean') {
      property['x-amesh-input'] = {
        type: input.type,
        sensitive: input.sensitive,
        placeholder: typeof input.placeholder === 'string' ? input.placeholder : null,
        prefill: input.prefill,
        maxBytes: typeof input.maxBytes === 'number' ? input.maxBytes : null,
      }
    }
    result[id] = property
  }
...
function inheritedTaskDefaults(pluginResolution: Record<string, unknown>): Record<string, InheritedTaskDefault> {
  const result: Record<string, InheritedTaskDefault> = {}
  const defaults = pluginResolution.defaults
  if (!isRecord(defaults) || !isRecord(defaults.tasks)) return result
  for (const [path, value] of Object.entries(defaults.tasks)) {
    if (!isRecord(value)) continue
    result[path] = {
...
export function FlowDetailPage({ session }: { session: UiSession }) {
  ...
  const properties = inputProperties(contract.data.inputSchema)
  const required = new Set(requiredInputs(contract.data.inputSchema))
  const setValue = (id: string, value: unknown) => setValues((current) => ({ ...current, [id]: value }))
  const defaultTasks = inheritedTaskDefaults(metadata.data.pluginResolution)
