...
import type { AgentEnvelopePreview, AgentResourceRevision, ArtifactRef, FlowTestRunResult, SecretBinding } from '../../api/types'
import type { WorkflowEditorSchema } from './workflowEditorSchemaModel'
import { openRouterModels } from '../agents'
              ...
              {task.type === 'agent.session' ? <details className="guided-step-advanced" open>
                ...
                <div className="guided-form-grid">
                  ...
                  {agentPreview ? <section className="simulation-summary span-two" aria-label="Resolved agent capability envelope">
                    ...
                    <dl><div><dt>Hard token ceiling</dt><dd>{agentPreview.envelope.hardLimits.maxTotalTokens?.toLocaleString() ?? '∞'}</dd></div><div><dt>Hard cost ceiling</dt><dd>${agentPreview.envelope.hardLimits.maxCostUsd ?? '∞'}</dd></div><div><dt>Duration ceiling</dt><dd>{agentPreview.envelope.hardLimits.maxDurationSeconds === null ? '∞' : `${agentPreview.envelope.hardLimits.maxDurationSeconds}s`}</dd></div><div><dt>Turn / tool ceiling</dt><dd>{agentPreview.envelope.hardLimits.maxTurns ?? '∞'} / {agentPreview.envelope.hardLimits.maxToolCalls ?? '∞'}</dd></div><div><dt>Memory</dt><dd>{agentPreview.envelope.memoryPolicy.scope} · {agentPreview.envelope.memoryPolicy.maxBytes.toLocaleString()} bytes</dd></div><div><dt>Permissions</dt><dd>{agentPreview.envelope.permissions.delegatedCapabilities.length} capabilities · {agentPreview.envelope.permissions.secretScopes.length} secret scopes</dd></div></dl>
