...
type PolicyRuleEditorDraft = PluginPolicyRuleDraft & { selector: NonNullable<PluginPolicyRuleDraft['selector']> }
...
export function PluginsPage({ session }: { session: UiSession }) {
  ...
  const [ruleDraft, setRuleDraft] = useState<PolicyRuleEditorDraft>({
          ...
          <>
            ...
            <div className="policy-rule-list" aria-label="Effective plugin policy rules">
              {policy.data.rules.length ? policy.data.rules.map((rule) => (
                <article key={rule.id ?? `${rule.scope}:${rule.priority}`}>
                  ...
                  <p>{rule.reason}</p>
                  <small>{rule.scope}{rule.namespace ? ` / ${rule.namespace}` : ''} · {rule.stages.join(', ')} · source {rule.id?.slice(0, 8) ?? 'unavailable'}</small>
                  {canManage && rule.id ? <button className="icon-button" type="button" aria-label={`Delete policy for ${rule.selector?.package ?? '*'}`} onClick={() => deleteRule.mutate(rule.id!)}><Trash2 size={15} /></button> : null}
                </article>
          ...
          {packages.map((release) => (
            <article className={`registry-card${release.yanked ? ' registry-card-yanked' : ''}`} key={`${release.name || 'legacy'}:${release.version || release.contentDigest}`}>
              ...
              <dl className="registry-facts">
                ...
                <div><dt>Downloads</dt><dd><Download size={14} aria-hidden="true" />{release.signals?.downloads ?? 0}</dd></div>
                <div><dt>Certification</dt><dd>{release.signals?.certification ?? 'unknown'}</dd></div>
