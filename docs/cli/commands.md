...
usage: amesh [-h] [--version] [--api-url API_URL] [--token TOKEN]
             ...
             {validate,apply,flows,executions,run,execution,logs,webhook,plugins,namespace,auth,config,flow,admin,lifecycle,upgrade,completion,command-docs,storage,recovery,tenant-transfer} ...
```
