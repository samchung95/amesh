        ...
        <div className="policy-rule-list" aria-label="Recent admission policy decisions">
          {decisions.data.slice(0, 12).map((decision) => (
            <article key={decision.id}>
              ...
              <small><Clock3 size={13} />{formatDate(decision.decidedAt, settings.locale, settings.timezone)} · {decision.flowId}@{decision.flowRevision} · {decision.evaluationDurationMs.toFixed(2)} ms · {decision.pinnedPolicies.length} pins</small>
            </article>
          ))}
