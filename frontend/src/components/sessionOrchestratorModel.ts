import type { AgentSessionFleetItem, AgentSessionFleetQuery, AgentSessionLifecycleState } from '../api/types'

export const MAX_BULK_SELECTION = 25

export type FleetStatusTone = 'running' | 'success' | 'failed' | 'warning' | 'waiting' | 'unknown'

export function fleetStatusTone(state: AgentSessionLifecycleState): FleetStatusTone {
...
export function formatFleetCost(value: string | number): string {
...
export function lifecycleActions(state: AgentSessionLifecycleState): Array<'cancel' | 'pause' | 'resume' | 'retry'> {
