...
from amesh.entrypoints.worker import (
    backfill_once,
...
async def _run_cycle(
    ...
    *,
    ...
    checks: CheckRepository | None = None,
    ...
    raise ValueError(f"role {role.value!r} must run through amesh.entrypoints.server")
...
async def run_role(settings: Settings, *, stop_event: asyncio.Event | None = None) -> None:
    ...
    if role is ServiceRole.WEBSERVER:
        raise ValueError("webserver role must run through python -m amesh.entrypoints.server")
