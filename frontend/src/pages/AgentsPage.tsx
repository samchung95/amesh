...
export function AgentsPage({ session }: { session: UiSession }) {
  ...
  const [envelope, setEnvelope] = useState<AgentCapabilityPin | null>(null)
  ...
  const resolve = useMutation({
    mutationFn: (resource: AgentResourceRevision) => api.resolveAgent(namespace, resource.key, resource.revision, `ui-preview:${crypto.randomUUID()}`),
  ...
  return (
    <div className="page agents-page">
      ...
      {selected ? <section className="agent-inspector"><div className="section-heading"><div><p className="eyebrow">EXACT REVISION</p><h2>{selected.spec.title}</h2></div><code>{selected.key}@{selected.revision}</code></div><div className="agent-inspector-actions">{selected.kind === 'AGENT' && session.capabilities['agents.execute'] ? <button className="button button-primary" type="button" disabled={resolve.isPending} onClick={() => resolve.mutate(selected)}><ShieldCheck size={16} />{resolve.isPending ? 'Resolving…' : 'Explain effective envelope'}</button> : null}{selected.kind === 'AGENT' && selected.revision > 1 ? <button className="button button-secondary" type="button" disabled={compare.isPending} onClick={() => compare.mutate(selected)}><GitCompareArrows size={16} />Compare with r{selected.revision - 1}</button> : null}</div>{resolve.error || compare.error ? <p className="form-error" role="alert">{(resolve.error || compare.error)?.message}</p> : null}<dl className="agent-revision-facts"><div><dt>Kind</dt><dd>{kindLabels[selected.kind]}</dd></div><div><dt>Digest</dt><dd><code>{selected.digest}</code></dd></div><div><dt>Created by</dt><dd>{selected.createdBy}</dd></div></dl><details><summary>Inspect canonical definition</summary><pre><code>{JSON.stringify(selected.spec, null, 2)}</code></pre></details></section> : null}

      {envelope ? <section className="agent-envelope"><div className="section-heading"><div><p className="eyebrow">PINNED BEFORE EXECUTION</p><h2>Effective capability envelope</h2></div><code>{envelope.envelopeDigest.slice(0, 22)}…</code></div><div className="agent-envelope-grid"><article><strong>{envelope.envelope.resources.length + 1}</strong><span>immutable resource pins</span></article><article><strong>{envelope.envelope.tools.length}</strong><span>allowed tools</span></article><article><strong>{envelope.envelope.hardLimits.maxTotalTokens}</strong><span>token ceiling</span></article><article><strong>${envelope.envelope.hardLimits.maxCostUsd}</strong><span>cost ceiling</span></article></div><ol className="agent-instruction-stack">{envelope.envelope.instructions.map((item) => <li key={`${item.sourceKind}:${item.sourceKey}`}><span>{item.order}</span><strong>{item.sourceKind}</strong><code>{item.sourceKey}</code></li>)}</ol><p className="permission-note">{envelope.envelope.outputNondeterminismDisclosure}</p></section> : null}
      {comparison ? <section className="agent-envelope"><div className="section-heading"><div><p className="eyebrow">REVISION DIFF</p><h2>r{comparison.fromRevision} → r{comparison.toRevision}</h2></div><span>{comparison.modelPolicyChanged ? 'Model policy changed' : 'Model policy unchanged'}</span></div><div className="agent-diff-grid"><article><strong>Added</strong><p>{[...comparison.addedPrompts, ...comparison.addedSkills, ...comparison.addedTools].join(', ') || 'Nothing'}</p></article><article><strong>Removed</strong><p>{[...comparison.removedPrompts, ...comparison.removedSkills, ...comparison.removedTools].join(', ') || 'Nothing'}</p></article></div><p className="permission-note">{comparison.nondeterminismDisclosure}</p></section> : null}
    </div>
  )
}
