...
export function AdministrationPage({ session }: { session: UiSession }) {
  ...
  const indexedAudit = useQuery({ queryKey: ['admin', 'indexed-audit'], queryFn: () => api.search({ query: '', types: ['AUDIT'], states: [], ranges: [], limit: 100, sort: 'OCCURRED_AT', direction: 'DESC' }), enabled: view === 'audit' })
...
function AccessAdministration({ api, tenant, namespaces, principals, roles, bindings, providers, pending, mutate }: { api: ReturnType<typeof useApiClient>; tenant: string; namespaces: string[]; principals: PrincipalDefinition[]; roles: Awaited<ReturnType<ReturnType<typeof useApiClient>['roles']>>; bindings: RoleBinding[]; providers: Awaited<ReturnType<ReturnType<typeof useApiClient>['providers']>>; pending: boolean; mutate: (operation: () => Promise<unknown>, message: string) => void }) {
  ...
  const submitBinding = (event: FormEvent) => { event.preventDefault(); const principal = principals.find((item) => item.id === bindingPrincipal); const principalId = principal?.id; if (!principal || !principalId) return; mutate(() => api.createBinding({ principal_id: principalId, principal_type: principal.principal_type, role_name: bindingRole, scope_type: bindingScope, tenant_id: bindingScope === 'INSTANCE' ? null : tenant, namespace: bindingScope === 'NAMESPACE' ? bindingNamespace.trim() : null }), 'Binding created') }
...
function OperationalPosture({ announcements, controls, api, namespace, namespaces, flows, workerGroups, plugins, onChanged, onFailure }: { announcements: Announcement[]; controls: OperationalControl[]; api: ReturnType<typeof useApiClient>; namespace: string; namespaces: string[]; flows: Array<{ namespace: string; flow_id: string }>; workerGroups: string[]; plugins: string[]; onChanged: (message: string) => Promise<void>; onFailure: (message: string) => void }) {
  ...
  const deactivateAnnouncement = (announcement: Announcement) => {
    if (!announcement.id) return
    void api.deactivateAnnouncement(announcement.id, announcement.version)
      .then(() => onChanged('Announcement deactivated'))
      ...
      <div className="admin-split equal">
        <section className="admin-panel">
          ...
          <div className="operational-list">
            {announcements.map((announcement) => <article key={announcement.id}><div><strong>{announcement.title}</strong><small>{announcement.severity} · {announcement.audience} · expires {new Date(announcement.expiresAt).toLocaleString()}</small><p>{announcement.message}</p></div><StatusBadge state={announcement.active ? 'RUNNING' : 'PAUSED'} />{announcement.active ? <button className="button button-quiet" type="button" onClick={() => deactivateAnnouncement(announcement)}>Deactivate</button> : null}</article>)}
          </div>
        </section>
        <section className="admin-panel">
          <div className="section-heading">
