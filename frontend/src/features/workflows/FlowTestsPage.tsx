...
function isFixtureMap(value: Record<string, unknown>): value is NonNullable<FlowTestDefinitionDraft['fixtures']> {
  return Object.values(value).every((fixture) => {
    if (!isRecord(fixture)) return false
    return typeof fixture.failuresBeforeSuccess === 'number'
      && ['INLINE', 'PLUGIN', 'RECORDED'].includes(typeof fixture.source === 'string' ? fixture.source : '')
  })
}

function parseFixtures(value: string): NonNullable<FlowTestDefinitionDraft['fixtures']> {
  const parsed = parseObject('Fixtures', value)
  if (!isFixtureMap(parsed)) throw new Error('Each fixture must declare source and failuresBeforeSuccess')
  return parsed
}

function isExpectation(value: Record<string, unknown>): value is NonNullable<FlowTestDefinitionDraft['expected']> {
  return ['FAILED', 'SKIPPED', 'SUCCESS'].includes(typeof value.state === 'string' ? value.state : '')
}

function parseExpectation(value: string): NonNullable<FlowTestDefinitionDraft['expected']> {
  const parsed = parseObject('Expected result', value)
  if (!isExpectation(parsed)) {
    throw new Error('Expected result state must be FAILED, SKIPPED, or SUCCESS')
  }
  return parsed
}
  ...
  const submit = (event: FormEvent) => {
    ...
    try {
      save.mutate({
        ...
        expected: parseExpectation(expected),
