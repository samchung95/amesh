...
import type { AgentSessionControlEvent, AgentSessionCreateRequest, AgentSessionControlSummary, UiSession } from '../api/types'
import { formatDate } from '../app/format'
import { useApiClient } from '../app/queries'
import { useAppSettings } from '../app/settings'
import { agentPinnedProfile, agentResourceOptions, currentHarnessAlias, harnessCatalogOptions, mergeSessionSummary, sessionCanCancel, sessionCanPause, sessionCanResume, sessionCanRetry, sessionEventLabel, sessionHarnessLabel, sessionIsLive } from '../components/agentSessionControlModel'
...
export function AgentSessionsPage({ session }: { session: UiSession }) {
  ...
  const canExecute = session.capabilities['executions.execute']
  const canManage = session.capabilities['executions.manage']
  const canViewSessions = session.capabilities['executions.view']
  ...
  const create = useMutation({
    mutationFn: (request: AgentSessionCreateRequest) => api.createAgentSession(request),
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      create.mutate({ agentRef, input: parsed as Record<string, unknown> })
    } catch (error) {
    ...
    <div className="page-stack agent-session-control-room">
      ...
      <div className="session-control-grid">
        <section className="data-section" aria-labelledby="start-session-heading">
          ...
          <form className="session-start-form" onSubmit={submit}>
            ...
            <button className="button button-primary" type="submit" disabled={create.isPending || !canExecute}><Play size={16} aria-hidden="true" />{create.isPending ? 'Starting…' : 'Start session'}</button>
      ...
      {active ? <SessionDetail session={active} result={result.data} events={events.data?.events || []} loading={detail.isPending || events.isPending} error={detail.error?.message || events.error?.message || result.error?.message || null} locale={settings.locale} timezone={settings.timezone} canManage={canManage} onAction={(action) => lifecycle.mutate({ action, id: active.sessionId, version: active.version ?? undefined, epoch: active.executionEpoch ?? undefined })} lifecyclePending={lifecycle.isPending} /> : <section className="data-section session-empty-detail"><History size={24} aria-hidden="true" /><div><h2>Choose a session to inspect</h2><p>The control room keeps run status, trace events, usage, budgets, and structured output in one bounded view.</p></div></section>}
    </div>
  )
}

function SessionDetail({ session, result, events, loading, error, locale, timezone, canManage, onAction, lifecyclePending }: { session: AgentSessionControlSummary; result?: { result: Record<string, unknown> | null; error: string | null }; events: AgentSessionControlEvent[]; loading: boolean; error: string | null; locale: string; timezone: string; canManage: boolean; onAction: (action: 'cancel' | 'pause' | 'retry' | 'resume') => void; lifecyclePending: boolean }) {
  ...
  return <section className="data-section session-detail" aria-labelledby="session-detail-heading">
    ...
    {current.budgets ? <div className="session-budget"><strong>Declared ceilings</strong><span>{current.budgets.maxTurns ?? '∞'} turns · {current.budgets.maxToolCalls ?? '∞'} tools · {current.budgets.maxTotalTokens?.toLocaleString() ?? '∞'} tokens · ${current.budgets.maxCostUsd ?? '∞'}</span></div> : null}
