...
export function ReleaseControlsPage({ session }: { session: UiSession }) {
  ...
  const loadTarget = async (kind: PromotionTargetKind, key: string) => {
    ...
    try {
      ...
      if (!isReleaseTarget(nextTarget) || !isReleaseHistory(nextHistory)) throw new Error('Release state response is malformed.')
  ...
  const previewPolicy = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!isPromotionGate(preview)) throw new Error('Release gate response is malformed.')
      setGate(preview)
  ...
  const applyPolicy = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!isReleaseAction(result)) throw new Error('Release action response is malformed.')
  ...
  const confirmRecovery = async () => {
    ...
    try {
      ...
      if (!isReleaseAction(result)) throw new Error('Release recovery response is malformed.')
      setTarget(result.target)
      setHistory((current) => [result.event, ...current])
