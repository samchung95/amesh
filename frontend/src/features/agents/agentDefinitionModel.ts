...
export function isAgentMcpToolCatalogEntry(value: unknown): value is AgentMcpToolCatalogEntry {
  return isRecord(value)
    && typeof value.connectionKey === 'string'
    && typeof value.connectionRevision === 'number'
    && typeof value.connectionDigest === 'string'
    && typeof value.credentialRef === 'string'
    && typeof value.endpoint === 'string'
    && typeof value.toolName === 'string'
    && typeof value.description === 'string'
    && typeof value.schemaDigest === 'string'
    && ['READ_ONLY', 'IDEMPOTENT_WRITE', 'HIGH_IMPACT'].includes(typeof value.impact === 'string' ? value.impact : '')
}

export function agentMcpToolCatalogEntries(values: unknown[]): AgentMcpToolCatalogEntry[] {
  const entries: AgentMcpToolCatalogEntry[] = []
  for (const value of values) {
    if (isAgentMcpToolCatalogEntry(value)) entries.push(value)
  }
  return entries
}
...
export function buildAgentResourceSpec(
  ...
  const networkHosts = [
    ...modelPolicy.spec.routes.map((route) => route.provider.endpoint),
    ...(evaluationJudgePolicy?.spec.kind === 'MODEL_POLICY' ? evaluationJudgePolicy.spec.routes.map((route) => route.provider.endpoint) : []),
    ...(tool ? [endpointHost(tool.endpoint)] : []),
  ].filter((value): value is string => typeof value === 'string' && value.length > 0)
    .map((value) => value.includes('://') ? endpointHost(value) : value)
    .filter((value, index, values) => values.indexOf(value) === index)
  return {
    kind: 'AGENT',
    ...shared,
