...
import { CatalogMultiSelect, CatalogSelect, ErrorState, LoadingState, type CatalogOption } from '../../shared/ui'
import { MissionControl } from './MissionControl'
import { missionControlModel, type MissionControlFilters } from './missionControlModel'
import {
...
export function DashboardPage({ session }: { session: UiSession }) {
  ...
  const principalOptions = Array.from(new Map([
    ...
    ...(principals.data || [])
      .filter((principal): principal is typeof principal & { id: string } => typeof principal.id === 'string')
      .map((principal) => [principal.id, { value: principal.id, label: `${principal.display_name} · ${principal.handle}` }] as const),
  ]).values())
