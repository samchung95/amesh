...
- [ ] **URS-F-0076** — The system shall address stored objects through opaque tenant-scoped URIs rather than local filesystem paths.
- [ ] **URS-F-0077** — The system shall support local development storage and S3-compatible production storage through one interface.
- [ ] **URS-F-0078** — The system shall stream uploads and downloads without loading large objects fully into process memory.
- [ ] **URS-F-0079** — The system shall record size, content type, checksum, encryption metadata, creator, retention and lineage.
- [ ] **URS-F-0080** — The system shall prevent cross-tenant access and path traversal at every storage boundary.
- [ ] **URS-F-0081** — The system shall support multipart upload, ranged download and resumable transfer where the backend permits.
- [ ] **URS-F-0082** — The system shall garbage-collect unreferenced objects only after a configurable safety window.
- [ ] **URS-F-0083** — The system shall verify object integrity on write and optionally on read.
