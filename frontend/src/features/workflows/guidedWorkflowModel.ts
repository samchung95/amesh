...
import type { AgentResourceRevision, ArtifactRef } from '../../api/types'
import { DEFAULT_OPENROUTER_MODEL } from '../agents'
...
function newTask(type: string, id: string, schema: WorkflowEditorSchema): Record<string, unknown> {
...
export function addGuidedStep(source: string, schema: WorkflowEditorSchema): string {
  const { document, data } = parseSource(source)
  const tasks = Array.isArray(data.tasks) ? data.tasks.filter(isRecord) : []
...
export function taskSupportsRunner(schema: WorkflowEditorSchema, type: string): boolean {
  const resource = schema.resourceCatalog.resources.find((item) => item.kind === 'task' && item.type === type)
  return Boolean(resource?.configurationSchema.properties?.taskRunner)
}

export function taskSupportsModel(schema: WorkflowEditorSchema, type: string): boolean {
  const resource = schema.resourceCatalog.resources.find((item) => item.kind === 'task' && item.type === type)
