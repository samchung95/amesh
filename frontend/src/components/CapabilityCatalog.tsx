...
import { ErrorState, LoadingState, EmptyState } from './AsyncState'
import {
  CAPABILITY_KINDS,
  capabilityCanAttach,
  capabilityExactRef,
    ...
    <section className="capability-catalog" aria-label="Capability catalog">
      ...
      <div className="capability-detail">
        {selected ? <>
          ...
          <dl className="capability-facts"><div><dt>Status</dt><dd>{capabilityStatusLabel(selected.status)}</dd></div><div><dt>Revision / version</dt><dd>{capabilityRevisionLabel(selected)}</dd></div><div><dt>Digest</dt><dd><code>{selected.digest}</code></dd></div><div><dt>Impact</dt><dd>{selected.impact}</dd></div><div><dt>Provider compatibility</dt><dd>{selected.providerCompatibility.join(', ') || 'Not declared'}</dd></div><div><dt>Permissions</dt><dd>{[...selected.permissions.delegatedCapabilities, ...selected.permissions.toolAllowlist, ...selected.permissions.secretScopes.map((scope) => `secret:${scope}`), ...selected.permissions.networkHosts.map((host) => `network:${host}`), ...selected.permissions.allowedEgress].join(', ') || 'None declared'}</dd></div></dl>
          <div className="capability-schemas">{Object.entries(selected.schemas).length ? Object.entries(selected.schemas).map(([name, schema]) => <section key={name}><h3>{name}</h3><pre>{JSON.stringify(schema, null, 2)}</pre></section>) : <p className="editor-empty">No schemas supplied.</p>}</div>
