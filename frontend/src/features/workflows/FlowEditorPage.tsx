...
export function FlowEditorPage({ session }: { session: UiSession }) {
  ...
  const savedNavigationTarget = useRef<{ namespace: string; flowId: string } | null>(null)
  ...
  const schema = useQuery({
    ...
    select: normalizeWorkflowEditorSchema,
  })
  ...
  useLayoutEffect(() => {
    ...
    const savedTarget = savedNavigationTarget.current
    if (savedTarget && existing && savedTarget.namespace === namespace && savedTarget.flowId === flowId) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void api.validateFlow(source).then((result) => {
        if (active) setValidation(editorValidation(result))
      }).catch((error: Error) => {
  ...
  const save = useMutation({
    ...
    onSuccess: (flow) => {
      ...
      if (!existing) {
        savedNavigationTarget.current = { namespace: flow.namespace, flowId: flow.flow_id }
        void navigate(`/flows/${encodeURIComponent(flow.namespace)}/${encodeURIComponent(flow.flow_id)}/edit`, { replace: true })
      }
    },
  })
  const format = useMutation({
    ...
    onSuccess: (result) => {
      setValidation(editorValidation(result.validation))
  ...
  const preflight = useMutation({
    mutationFn: async () => {
      ...
      setValidation(editorValidation(result))
  ...
  const isolatedTest = useMutation({
    mutationFn: async () => {
      ...
      const agentFixture: NonNullable<FlowTestDefinitionDraft['fixtures']> = guidedAgentTask ? {
        [guidedAgentTask.id]: {
          source: 'INLINE',
      ...
      const agentExpectation: Omit<NonNullable<FlowTestDefinitionDraft['expected']>, 'state'> = guidedAgentTask ? {
  ...
  if (!schema.data) return <ErrorState message="The flow editor schema is unavailable or malformed." retry={() => { void schema.refetch() }} />
