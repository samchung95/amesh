...
async def schedule_once(
    ...
    *,
    ...
    scheduled = 0
    for persisted_flow in await repository.list_flows(tenant_id="default"):
...
async def recover_once(repository: PostgresExecutionRepository, settings: Settings) -> int:
    now = datetime.now(UTC)
    recovered = 0
    for execution in await repository.list_executions(tenant_id="default", limit=1000):
        ...
        if (
            ...
            continue
        flow = await repository.get_flow(
            execution.namespace, execution.flow_id, tenant_id="default"
        )
        runner = KubernetesJobRunner.from_in_cluster(namespace=settings.kubernetes_task_namespace)
...
async def run_worker(settings: Settings) -> None:
    ...
    try:
        while True:
            await schedule_once(repository)
            await recover_once(repository, settings)
