...
import type { ArtifactRef, KeyValueType, NamespaceFileVersion, NamespaceResourceBundle, UiSession } from '../../api/types'
...
function isKeyValueType(value: unknown): value is KeyValueType {
  return value === 'STRING' || value === 'NUMBER' || value === 'BOOLEAN'
    || value === 'DATETIME' || value === 'DATE' || value === 'DURATION' || value === 'JSON'
}

function isNamespaceResourceBundle(value: unknown): value is NamespaceResourceBundle {
  return isRecord(value)
    ...
    && value.files.every((file) => isRecord(file) && typeof file.path === 'string' && typeof file.contentBase64 === 'string')
    && Array.isArray(value.keyValues)
    && value.keyValues.every((entry) => isRecord(entry) && typeof entry.key === 'string' && isKeyValueType(entry.type) && 'value' in entry)
    && Array.isArray(value.secrets)
    && value.secrets.every((secret) => isRecord(secret) && typeof secret.key === 'string' && typeof secret.provider === 'string' && typeof secret.providerReference === 'string')
}
...
export function NamespaceResourcesPage({ session }: { session: UiSession }) {
  ...
  const importBundle = async (file: File) => {
    try {
      ...
      if (!isNamespaceResourceBundle(bundle)) {
        setFailure('Resource bundle does not match the namespace resource schema')
        return
      }
      mutation.mutate(async () => {
