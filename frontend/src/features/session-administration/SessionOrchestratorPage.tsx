...
import { AgentProgressTimeline, progressImagesFromSessionEvents } from '../agent-sessions'
import { EmptyState, ErrorState, LoadingState } from '../../shared/ui'
import { buildFleetQuery, compactId, dependencyLabel, dependencyTone, fleetStatusLabel, fleetStatusTone, formatFleetCost, lifecycleActions, MAX_BULK_SELECTION, selectVisibleFleetRows, toggleFleetSelection } from './sessionOrchestratorModel'
...
function minimumBound(values: Array<number | null>): number | null {
  const bounded = values.filter((value): value is number => value !== null)
  return bounded.length ? Math.min(...bounded) : null
}

function minimumCost(values: Array<string | null>): string | null {
  const bounded = values.filter((value): value is string => value !== null)
  return bounded.length
    ? bounded.reduce((lowest, current) => Number(current) < Number(lowest) ? current : lowest)
    : null
}
...
function effectivePolicySpec(policies: AgentSessionPolicyRevision[]): AgentSessionPolicy {
  ...
  return {
    ...
    ceilingMode: specs.every((spec) => spec.ceilingMode === 'PROVIDER_BOUNDED') ? 'PROVIDER_BOUNDED' : 'BOUNDED',
    maxConcurrency: Math.min(...specs.map((spec) => spec.maxConcurrency)),
    maxTotalTokens: minimumBound(specs.map((spec) => spec.maxTotalTokens)),
    maxCostUsd: minimumCost(specs.map((spec) => spec.maxCostUsd)),
    maxDurationSeconds: minimumBound(specs.map((spec) => spec.maxDurationSeconds)),
...
function SessionPolicyPanel({ api, policies, loading, error, canManage, namespaceOptions, evaluationNamespace, evaluationApplication, onEvaluationNamespace, onEvaluationApplication, effectivePolicies, effectiveLoading, effectiveError, onSaved }: { api: ReturnType<typeof useApiClient>; policies: AgentSessionPolicyRevision[]; loading: boolean; error: string | null; canManage: boolean; namespaceOptions: string[]; evaluationNamespace: string; evaluationApplication: string; onEvaluationNamespace: (value: string) => void; onEvaluationApplication: (value: string) => void; effectivePolicies: AgentSessionPolicyRevision[]; effectiveLoading: boolean; effectiveError: string | null; onSaved: () => void }) {
  ...
  const edit = (policy: AgentSessionPolicyRevision) => {
    ...
    setDraft({
      ...policy.spec,
      maxTotalTokens: policy.spec.maxTotalTokens ?? emptyPolicy.maxTotalTokens,
      maxCostUsd: policy.spec.maxCostUsd ?? emptyPolicy.maxCostUsd,
      maxDurationSeconds: policy.spec.maxDurationSeconds ?? emptyPolicy.maxDurationSeconds,
...
function PolicyLimitGrid({ policy }: { policy: AgentSessionPolicy }) {
  return <dl className="policy-limit-grid"><div><dt>Admission</dt><dd>{policy.admissionEnabled ? 'Enabled' : 'Disabled'}</dd></div><div><dt>Limits</dt><dd>{policy.maxConcurrency} concurrent · {policy.maxTotalTokens?.toLocaleString() ?? '∞'} tokens · {policy.maxCostUsd === null ? '∞' : `$${policy.maxCostUsd}`}</dd></div><div><dt>Duration / retention</dt><dd>{policy.maxDurationSeconds === null ? '∞' : `${policy.maxDurationSeconds}s`} · {policy.retentionSeconds}s</dd></div><div><dt>Allowlists</dt><dd>Providers: {policyListValue(policy.allowedProviderIds)} · Harnesses: {policyListValue(policy.allowedHarnessIds)} · Tools: {policyListValue(policy.allowedToolIds)}</dd></div></dl>
}
