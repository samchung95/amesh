...
import { capabilityExactRef } from './capabilityCatalogModel'
...
export function AgentsPage({ session }: { session: UiSession }) {
  ...
  const tools = useQuery({
    ...
    queryFn: async () => agentMcpToolCatalogEntries((await Promise.all((connections.data || []).map((connection) => api.agentMcpTools(namespace, connection.spec.key, connection.revision)))).flat()),
