...
def test_compact_compose_has_one_amesh_process_and_only_postgresql_dependency() -> None:
    compact = _yaml("docker/compose.compact.yaml")
    ...
    assert runtime["command"][-1] == (
        "python -m amesh.entrypoints.migrations && exec python -m amesh.entrypoints.compact"
    )
...
def test_native_package_declares_compact_preflight_migration_and_resource_paths() -> None:
    ...
    assert scripts["amesh-compact"] == "amesh.entrypoints.compact:main"
    assert scripts["amesh-preflight"] == "amesh.entrypoints.preflight:main"
    assert scripts["amesh-migrate"] == "amesh.entrypoints.migrations:main"
