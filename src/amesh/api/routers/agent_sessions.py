...
async def _launch_agent_session(
    ...
    try:
        ...
        task_payload = {
            "id": "agent",
            "type": "agent.session",
            "dependsOn": [request.approval_task] if request.approval_task else [],
            "agent": request.agent,
        ...
        tasks = [task]
        if request.approval_task is not None:
            if request.approval_task == task.id:
                raise ValueError("approvalTask must differ from the canonical agent task ID")
            tasks.insert(
                0,
                TaskDefinition.model_validate(
                    {
                        "id": request.approval_task,
                        "type": "core.approval",
                        "title": f"Approve capabilities for {request.agent}",
                        "description": (
                            f"Agent: {namespace}/{request.agent}@{request.agent_revision}. "
                            f"Capability envelope: {preview.envelope_digest}. "
                            f"Logical session: {service_session_id}. "
                            "Review this execution's input and pinned capabilities before "
                            "approving. Approval applies only to this execution; each "
                            "follow-up requires a new decision."
                        ),
...
def _agent_session_follow_up_flow(
    ...
    return (
        source_flow.model_copy(
            update={
                "tasks": [
                    follow_up_task if task.id == source_task.id else task
                    for task in source_flow.tasks
                ]
            }
        ),
        agent_key,
